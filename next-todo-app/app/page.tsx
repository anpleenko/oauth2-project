  ...
  return (
    <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 bg-slate-50 space-y-2">
      ...
      <div className="flex justify-end">
        <a href="http://localhost/oauth2/sign_out?rd=http%3A%2F%2Flocalhost%3A3000%2F">
          <p className="text-blue-500 underline transition cursor-pointer">
            Logout
          </p>
        </a>
      </div>
      {/* logout button end */}
      <h1 className="text-center text-2xl sm:text-3xl md:text-4xl font-bold text-slate-600 my-8">
        📝 Simple Todo Service
      </h1>
      ...
      <div className="space-y-2">
        {todos?.filter(incompletedTodos).map((todo: any, index: number) => (
          <div
            ...
            key={index + 1}
          >
            <p className="text-slate-500 truncate max-w-[190px] md:max-w-[500px] md:text-lg">
              {todo.title}
            </p>
      ...
      <div className="space-y-2">
        {todos?.filter(completedTodos).map((todo: any, index: number) => (
          <div
            ...
            key={index + 1}
          >
            <p className="text-slate-500 truncate max-w-[190px] md:max-w-[500px] md:text-lg line-through">
              {todo.title}
            </p>
            <div className="space-x-2 flex items-center">
              <div
